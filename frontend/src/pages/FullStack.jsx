...
const fsd = {
  ...
  sections: {
    student_hook:
      "Full-stack means bridging pixels to databases. Build cohesive features end-to-end and ship value fast.",
    integration: {
      concept:
        "Integrate frontend and backend through clear contracts (REST/GraphQL), typed DTOs, and consistent error handling.",
      realWorldExample:
        "A BFF (Backend For Frontend) shapes data for a specific UI, reducing over-fetching.",
      industry_applications: ["🧩 BFF","🔌 API aggregation","🧭 Monorepos","🧰 Shared types"],
      advantages: ["⚡ Velocity","🧭 Ownership","🔁 Fewer handoffs","🧰 Monorepo DX"],
      disadvantages: ["🧮 Context switching","🧱 Breadth over depth","🧩 Tooling sprawl","🔧 Infra overlap"],
    },
    flows: {
      concept:
        "Design feature flows: auth → UI → API → DB → events. Automate with CI and environments.",
      realWorldExample:
        "A checkout flow validates inputs client/server and emits an 'order.created' event.",
      industry_applications: ["🛒 Checkout","🧾 Onboarding","🧠 Recommendations","📣 Notifications"],
      advantages: ["🧭 Clarity","🧪 Testability","🔁 Reuse","📈 Observability"],
      disadvantages: ["🧩 Coordinating changes","🧱 Version drift","🧮 Data mapping","🧪 E2E fragility"],
    },
    tooling: {
      concept:
        "Full-stack DX: monorepos, codegen, shared UI kits, type-safe RPC, and preview deployments.",
      realWorldExample:
        "Type-safe API client is generated from the server schema for zero-mismatch calls.",
      industry_applications: ["📦 Packages","🧱 Workspaces","🧰 Codegen","🚀 Previews"],
      advantages: ["⚡ DX","🧭 Consistency","🧩 Reuse","🧪 Confidence"],
      disadvantages: ["🧱 Setup cost","🔧 Tool upgrades","🧩 Compatibility","📦 Build times"],
    },
    code_examples: {
      javascript: `// Shared types (ts-like idea)\n/** User */\n// export type User = { id: string; email: string };`,
      sql: `-- Create orders\nCREATE TABLE orders(id uuid primary key, total numeric);`,
    },
    interview_questions: [
      { question: "When BFF vs direct API?", difficulty: "Medium", answer: "Use BFF to tailor responses to UI and hide service topology; direct when simple and uniform." },
      { question: "Avoid tight coupling?", difficulty: "Medium", answer: "Define contracts, version APIs, keep UI and API independent deploys where possible." },
    ],
    project_ideas: [
      { title: "E2E Feature Slice", difficulty: "Beginner", description: "Add a feature across UI/API/DB with tests.", technologies: ["React","Node","Postgres"] },
      { title: "Type-safe Client", difficulty: "Intermediate", description: "Generate client from schema.", technologies: ["OpenAPI","Codegen"] },
    ],
  },
};
...
export default function FullStack(){
  const { topic, category, sections } = fsd;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-indigo-900/20 text-gray-900 dark:text-white">
      ...
      <footer className="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-12 mt-16"><div className="max-w-7xl mx-auto px-6 text-center"><h3 className="text-2xl font-bold mb-4">Own the stack 🧱</h3><p className="text-lg text-gray-300 mb-6">Ship end-to-end features with confidence.</p></div></footer>
    </div>
  );
}
