...
const pwa = {
  topic: "Mobile & Progressive Web Apps (PWAs)",
  category: "Offline • Installable • Push",
  sections: {
    student_hook:
      "Bring app-like capabilities to the web: offline, installable, fast re-engagement.",
    offline: {
      concept:
        "Service workers cache assets and data for offline and fast loads. Cache strategies balance freshness and speed.",
      realWorldExample:
        "An app precaches shell and uses stale-while-revalidate for content.",
      industry_applications: ["📡 Low-connectivity","📱 Installable","🔁 Sync","🧭 Shell"],
      advantages: ["⚡ Instant","🧭 Resilience","📉 Data usage","📱 App-like"],
      disadvantages: ["🔧 SW complexity","🧪 Cache bugs","🧱 Storage","🔔 Permissions"],
    },
    install: {
      concept:
        "Manifests define icons, name, and display; users can install PWAs; push notifications re-engage.",
      realWorldExample:
        "An e-commerce PWA sends restock notifications.",
      industry_applications: ["🔔 Push","📱 Homescreen","🧭 Fullscreen","🧰 Shortcuts"],
      advantages: ["📈 Engagement","⚡ Speed","📱 Presence","💸 Cross-platform"],
      disadvantages: ["🔔 Consent","🧱 Platform limits","🔧 Setup","🧪 Testing"],
    },
    api: {
      concept:
        "Web APIs (storage, camera, geolocation) enable powerful capabilities, gated by permissions.",
      realWorldExample:
        "A field app uses camera and geolocation with offline sync.",
      industry_applications: ["📍 Maps","📸 Capture","🗃️ Storage","🔌 Sensors"],
      advantages: ["🧰 Capability","📱 UX","🔗 Integrations","⚡ Performance"],
      disadvantages: ["🔐 Privacy","🧱 Platform gaps","🧪 Permission UI","🔧 Limits"],
    },
    code_examples: {
      javascript: `// Register SW\nif('serviceWorker' in navigator){navigator.serviceWorker.register('/sw.js')}`,
      json: `{
  "name": "App",
  "short_name": "App",
  "start_url": "/",
  "display": "standalone"
    ...
    interview_questions: [
      { question: "Cache strategies?", difficulty: "Medium", answer: "Network-first, cache-first, stale-while-revalidate depending on content." },
      { question: "PWA install criteria?", difficulty: "Medium", answer: "Valid manifest, served over HTTPS, registered service worker, engagement." },
    ],
    project_ideas: [
      { title: "Offline Notes", difficulty: "Beginner", description: "Notes app with offline and sync.", technologies: ["SW","IndexedDB"] },
      { title: "Push Demo", difficulty: "Intermediate", description: "Subscribe and send test notifications.", technologies: ["Web Push","VAPID"] },
    ],
  },
};
...
export default function WebPWA(){
  const { topic, category, sections } = pwa;
  const [lang, setLang] = useState('javascript');
  const [openQ, setOpenQ] = useState(-1);
  const langs=['javascript','json'];
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-indigo-900/20 text-gray-900 dark:text-white">
      ...
      <footer className="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-12 mt-16"><div className="max-w-7xl mx-auto px-6 text-center"><h3 className="text-2xl font-bold mb-4">App-like web 📱</h3><p className="text-lg text-gray-300 mb-6">Offline-first and installable experiences.</p></div></footer>
    </div>
  );
}
