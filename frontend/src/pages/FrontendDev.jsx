...
const fe = {
  ...
  category: "UI • State • Performance",
  sections: {
    student_hook:
      "Frontend is where users feel your product. Build accessible, fast, and beautiful UIs.",
    ui: {
      concept:
        "Components, routing, forms, and accessibility form the core. Design systems ensure consistency.",
      realWorldExample:
        "A component library accelerates development across squads.",
      industry_applications: ["🧩 Design systems","🧭 SPA routing","♿ A11y","🧰 Reuse"],
      advantages: ["⚡ Velocity","♿ Inclusive","🧱 Consistency","🧰 Ecosystem"],
      disadvantages: ["📦 Bundle size","🧮 State sprawl","🧑‍🔬 Testing complexity","🧱 Fragmentation"],
    },
    state: {
      concept:
        "State management via local/component state, server cache, or global stores; query libraries for data-fetching.",
      realWorldExample:
        "React Query caches server data and handles stale/refresh automatically.",
      industry_applications: ["📡 Data sync","🔁 Cache","🧭 URL state","🧰 Stores"],
      advantages: ["⚡ UX","📉 Fewer calls","🧭 Predictable","🧪 Testable"],
      disadvantages: ["🧮 Complexity","🔁 Invalidation","🧱 Boilerplate","🧩 Tool lock-in"],
    },
    perf: {
      concept:
        "Performance via code-splitting, lazy loading, memoization, virtualization, and profiling.",
      realWorldExample:
        "Route-based code-splitting reduces initial bundle by 60%.",
      industry_applications: ["⚡ Core Web Vitals","📦 CDN","🧠 Memo","🧰 Profiling"],
      advantages: ["🚀 Faster","📈 SEO","📉 Cost","🧑‍🦽 Better UX"],
      disadvantages: ["🧩 Complexity","🧪 Edge cases","🧱 Over-optimizing","🔧 Maintenance"],
    },
    code_examples: {
      javascript: `// Lazy component\nconst Page = React.lazy(()=>import('./Page'));`,
      html: `<!-- Accessible button -->\n<button aria-label="Open menu">☰</button>`,
      css: `/* Prefers-reduced-motion */\n@media (prefers-reduced-motion: reduce){*{animation:none}}`,
    },
    interview_questions: [
      { question: "Fix slow list?", difficulty: "Medium", answer: "Virtualize, memoize item render, key properly, avoid re-renders." },
      { question: "A11y basics?", difficulty: "Easy", answer: "Semantic HTML, labels, focus management, contrast, keyboard nav." },
    ],
    project_ideas: [
      { title: "Design System", difficulty: "Intermediate", description: "Build tokens, components, docs.", technologies: ["Storybook","CSS vars"] },
      { title: "Virtualized Table", difficulty: "Intermediate", description: "Render 100k rows smoothly.", technologies: ["React","Virtualizer"] },
    ],
  },
};
...
export default function FrontendDev(){
  const { topic, category, sections } = fe;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-indigo-900/20 text-gray-900 dark:text-white">
      ...
      <footer className="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-12 mt-16"><div className="max-w-7xl mx-auto px-6 text-center"><h3 className="text-2xl font-bold mb-4">Delight users ✨</h3><p className="text-lg text-gray-300 mb-6">Fast, accessible, and robust frontends win.</p></div></footer>
    </div>
  );
}
