...
const rl = {
  ...
  category: "Agents, Rewards, Policies",
  sections: {
    student_hook:
      "RL learns by doing: agents explore, receive rewards, and improve strategies over time.",
    fundamentals: {
      concept:
        "MDPs define states, actions, rewards, transitions. Value functions estimate returns; policies map states to actions.",
      realWorldExample:
        "A robot learns navigation by maximizing goal-reaching and minimizing collisions.",
      industry_applications: ["🤖 Robotics control","🎮 Game AI","💹 Portfolio allocation","🚚 Supply chain"],
      advantages: ["🧭 Sequential decisions","📈 Long-horizon","🧠 Strategy learning","🔁 Online adaptation"],
      disadvantages: ["🧪 Sample-inefficient","🌀 Instability","🎯 Reward hacking","⚖️ Safety"],
    },
    methods: {
      concept:
        "Value-based (Q-learning), policy gradients, actor-critic balance bias/variance and stability.",
      realWorldExample:
        "PPO clips policy updates to stabilize training.",
      industry_applications: ["📦 Picking/placing","🛰️ Satellite ops","🚗 Driving policies","🧪 Experiment design"],
      advantages: ["🛠️ Versatile","🧮 Model-free","📐 Discrete/continuous","🧩 Function approx"],
      disadvantages: ["⚙️ Sensitive tuning","🐢 Slow","🔀 Non-stationary","🧠 Credit assignment"],
    },
    practice: {
      concept:
        "Reward shaping, exploration schedules, domain randomization, constraints improve performance and safety.",
      realWorldExample:
        "Curriculum learning starts easy, then increases difficulty.",
      industry_applications: ["🧪 Constrained RL","🧭 Sim2Real","📊 Bandits","⚙️ Real-time control"],
      advantages: ["🧑‍🏫 Guided","🧪 Robust","📈 Better exploration","🔐 Safer"],
      disadvantages: ["🔧 Engineering cost","🧮 More compute","🧪 Overfit to reward","🧱 Env design"],
    },
    code_examples: {
      python: `# Q-learning update\nQ[s,a] = Q[s,a] + alpha*(r + gamma*max_a' Q[s',a'] - Q[s,a])`,
      javascript: `// epsilon-greedy\nfunction act(q,eps){ if(Math.random()<eps) return rand(); return argmax(q); }`,
      c: `// Discounted return\ndouble G(double r, double g, double next){ return r + g*next; }`,
      cpp: `// Soft update\ndouble mix(double t, double src, double tau){ return (1-tau)*t + tau*src; }`,
      java: `// Argmax\nint argmax(double[] x){int b=0;for(int i=1;i<x.length;i++) if(x[i]>x[b]) b=i;return b;}`,
    },
    interview_questions: [
      { question: "Why PPO popular?", difficulty: "Medium", answer: "Clipped objective limits destructive updates while enabling stable improvement." },
      { question: "Exploration strategies?", difficulty: "Medium", answer: "Epsilon-greedy, entropy bonuses, intrinsic rewards, parameter noise." },
    ],
    project_ideas: [
      { title: "CartPole PPO", difficulty: "Beginner", description: "Train PPO with reward shaping.", technologies: ["Gymnasium","PyTorch"] },
      { title: "Robot Arm Sim2Real", difficulty: "Advanced", description: "Domain randomization for robust policies.", technologies: ["DM Control","ROS"] },
    ],
  },
...
const highlightSyntax = (code, language) => {
  const keys = { python:['alpha','gamma'], javascript:['function','return','if'], c:['double','return'], cpp:['double','return'], java:['int','return'] };
...
export default function ReinforcementLearning(){
  const { topic, category, sections, envs } = rl;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-indigo-900/20 text-gray-900 dark:text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 space-y-16">
        ...
        <section><h2 className="text-4xl font-bold mb-8 text-center">🧪 Common Environments</h2><div className="flex flex-wrap gap-2 justify-center">{envs.map((e,i)=>(<span key={i} className="px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-sm">{e}</span>))}</div></section>
      ...
      <footer className="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-12 mt-16"><div className="max-w-7xl mx-auto px-6 text-center"><h3 className="text-2xl font-bold mb-4">Reward good behavior, safely 🧭</h3><p className="text-lg text-gray-300 mb-6">Stability, safety, and sample-efficiency are key.</p></div></footer>
    </div>
  );
}
