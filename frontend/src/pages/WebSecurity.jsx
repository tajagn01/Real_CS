...
const sec = {
  topic: "Security in Web Development",
  category: "OWASP • HTTPS • Secure Auth",
  sections: {
    student_hook:
      "Secure by design: protect users and systems from common vulnerabilities and threats.",
    owasp: {
      concept:
        "Top risks include XSS, CSRF, SQLi, auth flaws, and misconfig. Sanitize, validate, encode, and use prepared statements.",
      realWorldExample:
        "CSP and escaping mitigate XSS; CSRF tokens protect state-changing requests.",
      industry_applications: ["🛡️ WAF","🧪 SAST/DAST","🔐 Secrets","📜 Policies"],
      advantages: ["🛡️ Risk reduction","📜 Compliance","🤝 Trust","🧭 Governance"],
      disadvantages: ["🔧 Overhead","🧱 False positives","🧪 Maintenance","🧩 Complexity"],
    },
    https: {
      concept:
        "HTTPS/TLS, HSTS, secure cookies, and headers (CSP, X-Frame-Options) harden web apps.",
      realWorldExample:
        "HSTS enforces HTTPS; Secure and HttpOnly cookies protect sessions.",
      industry_applications: ["🔐 TLS","🧭 HSTS","🍪 Cookies","🪪 Headers"],
      advantages: ["🛡️ Confidentiality","🧾 Integrity","🧪 Hardening","🔍 Audit"],
      disadvantages: ["🔧 Cert mgmt","🧱 Legacy","🧩 Mixed content","🧪 Misconfig"],
    },
    auth: {
      concept:
        "JWT/OAuth/OIDC flows, password hashing, MFA, and rate-limiting protect identities.",
      realWorldExample:
        "PKCE in OAuth protects public clients; MFA reduces account takeover.",
      industry_applications: ["🔑 OIDC","🛂 RBAC/ABAC","🔐 Hashing","🛡️ Rate limits"],
      advantages: ["🛡️ Security","🤝 Federation","🧭 Least privilege","🧾 Auditing"],
      disadvantages: ["🧩 Complexity","🔗 Third-parties","🧪 Edge flows","🧱 Recovery"],
    },
    code_examples: {
    ...
    interview_questions: [
      { question: "Prevent XSS?", difficulty: "Medium", answer: "Encode output, CSP, sanitize input, avoid dangerouslySetInnerHTML, use frameworks safely." },
      { question: "Secure cookies?", difficulty: "Medium", answer: "Set Secure, HttpOnly, SameSite, use short TTLs, and rotate secrets." },
    ],
    project_ideas: [
      { title: "Security Headers", difficulty: "Beginner", description: "Add and validate security headers.", technologies: ["Helmet","CSP"] },
      { title: "Auth Hardening", difficulty: "Intermediate", description: "Enable MFA and rate-limiting.", technologies: ["OIDC","Limiter"] },
    ],
  },
};
...
export default function WebSecurity(){
  const { topic, category, sections } = sec;
