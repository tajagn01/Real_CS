...
const rai = {
  topic: "Ethics, Bias & Responsible AI",
  ...
  sections: {
    ...
    code_examples: {
      ...
      javascript: `// Fairness metric placeholder\nfunction dp(p_pos_groupA,p_pos_groupB){return Math.abs(p_pos_groupA-p_pos_groupB);}`,
      c: `// Privacy budget (placeholder)\ndouble epsilon(double s){return s;}`,
      cpp: `// Clip value\ndouble clip(double x,double a,double b){return x<a?a:(x>b?b:x);}`,
      java: `// Audit log\nvoid audit(String msg){ System.out.println(msg); }`,
    },
    interview_questions: [
...
const highlightSyntax = (code, language) => {
  const keys = { python:['import'], javascript:['function','return'], c:['double','return'], cpp:['double','return'], java:['void','return'] };
...
export default function ResponsibleAI(){
  const { topic, category, sections, tools } = rai;
