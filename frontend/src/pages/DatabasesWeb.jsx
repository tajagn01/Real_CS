...
const db = {
  topic: "Databases & Data Handling",
  category: "SQL • NoSQL • Caching",
  sections: {
    student_hook:
      "Pick the right storage for the job: relational for transactions, NoSQL for scale, caches for speed.",
    relational: {
      concept:
        "SQL databases (Postgres/MySQL) offer ACID transactions, schemas, joins, and strong consistency.",
      realWorldExample:
        "An order system uses Postgres with indexes and constraints to ensure integrity.",
      industry_applications: ["💳 Payments","🧾 Orders","📊 Analytics","🧭 OLTP"],
      advantages: ["🧭 Consistency","🔐 Integrity","🧰 Maturity","🧪 SQL"],
      disadvantages: ["🧮 Vertical scale","🧱 Sharding","🧩 Migrations","📈 Cost"],
    },
    nosql: {
      concept:
        "NoSQL (Mongo, Dynamo, Redis) scales horizontally with flexible schemas and specialized models.",
      realWorldExample:
        "A feed uses Redis cache and Mongo for flexible documents.",
      industry_applications: ["⚡ Caching","🧵 Streams","📦 Docs","🧭 Key-value"],
      advantages: ["📈 Scale-out","⚡ Latency","🧩 Flexibility","🧰 Specialized"],
      disadvantages: ["🧭 Consistency trade-offs","🧱 Modeling","🔁 Secondary indexes","🧪 Transactions"],
    },
    code_examples: {
      sql: `-- Join\nSELECT u.id, o.total FROM users u JOIN orders o ON o.user_id=u.id`,
      javascript: `// Redis get\nconst v = await redis.get('key')`,
    },
    interview_questions: [
      { question: "When SQL vs NoSQL?", difficulty: "Medium", answer: "Strong consistency and relations vs flexibility/scale; often hybrid with caches." },
      { question: "Index trade-offs?", difficulty: "Medium", answer: "Faster reads, slower writes, more storage; choose selective indexes." },
    ],
    project_ideas: [
      { title: "Read Replica Setup", difficulty: "Intermediate", description: "Scale reads with replicas and failover.", technologies: ["Postgres","Patroni"] },
      { title: "Cache Layer", difficulty: "Beginner", description: "Add Redis cache with TTL and metrics.", technologies: ["Redis","Node"] },
    ],
  },
};
...
export default function DatabasesWeb(){
  const { topic, category, sections } = db;
  const [lang, setLang] = useState('sql');
  const [openQ, setOpenQ] = useState(-1);
  const langs=['sql','javascript'];
