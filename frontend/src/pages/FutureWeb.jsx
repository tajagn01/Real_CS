...
const futureWeb = {
  ...
  category: "Web3 • AI-driven Web • Immersive Experiences",
  sections: {
    student_hook:
      "The web keeps evolving: smarter apps, decentralized backends, and immersive 3D/AR experiences.",
    ai: {
      concept:
        "AI-native apps use LLMs for reasoning and generation, with RAG, agents, and copilots embedded in UIs.",
      realWorldExample:
        "Docs with AI copilots summarize content and draft sections inline.",
      industry_applications: ["🤖 Copilots","📄 Smart docs","🧠 Personalization","🔎 Semantic search"],
      advantages: ["⚡ Productivity","🧠 Intelligence","📈 Engagement","🧰 Tooling"],
      disadvantages: ["💸 Cost","🧪 Eval","🔐 Privacy","🧱 Vendor"],
    },
    web3: {
      concept:
        "Web3 apps use wallets, smart contracts, and decentralized storage for ownership and composability.",
      realWorldExample:
        "A marketplace mints passes as NFTs for member features.",
      industry_applications: ["💳 Payments","🎟️ Membership","🧠 Identity","📦 Storage"],
      advantages: ["🧾 Ownership","🔗 Composability","🌍 Open infra","🔐 Verifiability"],
      disadvantages: ["🧱 UX hurdles","💸 Fees","🔐 Keys","📈 Volatility"],
    },
    immersive: {
      concept:
        "WebGPU/WebGL power 3D/AR/VR in browsers, enabling interactive scenes and simulations.",
      realWorldExample:
        "A training app simulates factory floors in 3D for safety drills.",
      industry_applications: ["🎮 3D UIs","🏭 Simulation","🧰 CAD","🎓 Training"],
      advantages: ["🎨 Rich UIs","⚡ GPU","🧩 Interactivity","🌐 Portability"],
      disadvantages: ["🧮 Complexity","📦 Assets","🧱 Compatibility","🧪 Performance tuning"],
    },
    code_examples: {
      javascript: `// WebGPU check\nconst ok = 'gpu' in navigator;`,
      html: `<!-- Wallet connect button -->\n<button>Connect</button>`,
      css: `/* 3D canvas */\ncanvas{width:100%;height:400px}`,
    },
    interview_questions: [
      { question: "When to use AI in the web stack?", difficulty: "Medium", answer: "Text-heavy UX, summarization, copilots, and search benefit most; ground with retrieval." },
      { question: "Web3 pros/cons?", difficulty: "Medium", answer: "Ownership/composability vs UX/cost/compliance; pick per use case." },
    ],
    project_ideas: [
      { title: "AI Doc Copilot", difficulty: "Beginner", description: "Inline summarization and drafting.", technologies: ["LLM API","RAG"] },
      { title: "3D Product Viewer", difficulty: "Intermediate", description: "Orbit controls and variants.", technologies: ["Three.js","WebGPU"] },
    ],
  },
};

const highlightSyntax = (code) => {
  const strings= /"[^"]*"|'[^']*'|`[^`]*`/g; return code.replace(strings,m=>`<span style="color:#22c55e;">${m}</span>`);
};

export default function FutureWeb(){
  const { topic, category, sections } = futureWeb;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-indigo-900/20 text-gray-900 dark:text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 space-y-16">
        <section><h2 className="text-4xl font-bold mb-6 text-center bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">🔮 Where the web is going</h2><div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl border-l-8 border-blue-500"><p className="text-xl leading-relaxed text-gray-700 dark:text-gray-200 italic">{sections.student_hook}</p></div></section>

        {['ai','web3','immersive'].map((key, idx)=>{
          const sec=sections[key]; const meta=[{emoji:'🤖',pill:'from-blue-600 to-indigo-600',border:'border-blue-500',tag:'text-blue-700 dark:text-blue-300',bg:'bg-blue-50 dark:bg-blue-900/20',bar:'border-blue-400'},{emoji:'🕸️',pill:'from-green-600 to-emerald-600',border:'border-green-500',tag:'text-green-700 dark:text-green-300',bg:'bg-green-50 dark:bg-green-900/20',bar:'border-green-400'},{emoji:'🎮',pill:'from-purple-600 to-violet-600',border:'border-purple-500',tag:'text-purple-700 dark:text-purple-300',bg:'bg-purple-50 dark:bg-purple-900/20',bar:'border-purple-400'}][idx]; const title= key==='ai'?'AI-driven Web': key==='web3'?'Web3 & Decentralization':'Immersive Web';
      ...
      <footer className="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-12 mt-16"><div className="max-w-7xl mx-auto px-6 text-center"><h3 className="text-2xl font-bold mb-4">Ship the next-gen web 🚀</h3><p className="text-lg text-gray-300 mb-6">Blend AI, decentralization, and immersion responsibly.</p></div></footer>
    </div>
  );
}
